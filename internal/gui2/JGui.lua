...
jGui = {
  ...
  mouse = require("gui2.mouse"),
  kb = require("gui2.jKeyboard"),
...
function jGui:init()
  ...
  -- set a pointer to its window so that it can be accessed easilly later
  local window = reaper.JS_Window_Find(self.title, true)
  self.window_pointer = window
end
...
function jGui:setTitle(new_name)
  local title = reaper.JS_Window_GetTitle(self.window_pointer)
  if title then
    reaper.JS_Window_SetTitle(self.window_pointer, new_name)
  end
end
...
function jGui:setFocus(c)
  ...
  if self.context_helpers then
    log.user("jGui:setFocus -> self.context_helpers")
    local cxh = self.context_helpers
    if cxh.on_focus_change then
      for _, v in ipairs(cxh.on_focus_change) do
...
-- 	list = {},
-- }
-- TODO: rename this to `set_last_focused_track`
function jGui:set_last_selection_idx(idx)
  self.lastSelIdx = idx
  log.user("set last idx:", self.lastSelIdx)
end
...
function jGui:get_selection()
  log.user("last sel idx =", self.lastSelIdx)

  if self:has_mult_select() then
    return self:get_mult_select()
  else
    local res = self:get_on_enter_selection()
    log.user("res ->", format.block(res))
    return { res }
  end
end
