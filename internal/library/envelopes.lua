...
envelopes.loop_midi_buf_string = function()
  ...
  while stringPos < MIDIlen - 12 do                                               -- parse through all events in the MIDI string, one-by-one, excluding the final 12 bytes, which provides REAPER's All-notes-off end-of-take message
    ...
    then
      ...
      msg_cc_lane = msg:sub(1, 1) ..
          string.char(cc_lane) ..
          msg:sub(3, 3)                                                                                        -- write msg chunk for cc_lane
...
envelopes.midi_take_fltr_cc = function(opts)
  ...
  -- DELETE -> local ret = reaper.MIDI_DeleteCC( take, ccidx )
  --
      ...
      elseif k == "pitch" then
        ...
        for i, evt in ipairs(list_cc) do
          ...
          if evt.shape then
            ...
            reaper.MIDI_SetCCShape(opts.take, get_cc_count() - 1, 1, 0, true)
            log.user("setting shape of idx =", ccevtcnt)
          end
        end
      end
    end
