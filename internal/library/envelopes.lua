  ...
  return function()
    ...
    if prev_type then
      ...
      then
        ...
        log.user(string.format([[ERROR(enum curve nodes): %s -> %s]], prev_type ~= nil and prev_type or "nil", node_type))
      end
    else
...
envelopes.delete = function(env)
  reaper.PreventUIRefresh(1)
  require("library.delete_envelope")(env)
  reaper.PreventUIRefresh(-1)
  reaper.UpdateArrange()
end
...
envelopes.curve_obj_already_exists_at_position = function(env, range_left, range_right, is_midi, midi_take, midi_type,
                                                          cc_num)
...
envelopes.insert_from_template = function(trobj, t_template, target_def, range_left, range_right, midi_target_take)
  local target_tr = trobj.tr
  local target_env
  local t_pts_to_insert
  local is_cc = target_def.cc

  -- Get track envs if possible
  if target_def.code == "fx" then
    target_env = reaper.GetFXEnvelope(trobj.tr, target_def.fx_idx, target_def.fx_param.index, true)
  elseif not target_def.cc then
    target_env = reaper.GetTrackEnvelopeByChunkName(trobj.tr,
      constants.BUILTIN_ENVELOPES[target_def.code].search_string)
  end

  -- Get midi info if possible
  local target_midi_type
  local cc_num
  if target_def.code == "pitch_bend" then
    target_midi_type = "pitch"
  elseif target_def.code:match("^cc_") then
    target_midi_type = "cc"
    cc_num = tonumber(target_def.code:match("_(%d+)$"))
    if not cc_num then
      cc_num = 1
    end
  end
  ...
  local creates_overlap = envelopes.curve_obj_already_exists_at_position(target_env, range_left, range_right, is_cc,
