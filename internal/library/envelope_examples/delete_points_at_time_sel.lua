--
-- NOTE: This is a good eample of how to generate and insert new points at
-- interpolated values so that one can easilly chunk up the curve timeline.
-- and then.
--
--[[
...
local function DeleteAtTimeSelection()
    if time_selection == true then
        ...
        retval_last, time_last, valueSource_last, shape_last, tension_last, selectedOut_last =
            reaper.GetEnvelopePoint(env, env_points_count - 1)
        reaper.DeleteEnvelopePointRange(env, 0, time_last + 1)
    end
end

local function Action(env)
    local _, valueOut3, dVdSOutOptional3, ddVdSOutOptional3, dddVdSOutOptional3

    local _, valueOut4, dVdSOutOptional4, ddVdSOutOptional4, dddVdSOutOptional4
    ...
    if time_selection == true and preserve_edges == true then -- IF we want to preserve edges of time selection
        _, valueOut3, _, _, _ = reaper.Envelope_Evaluate(env, start_time, 0, 0)
        _, valueOut4, _, _, _ = reaper.Envelope_Evaluate(env, end_time, 0, 0)
    end -- preserve edges of time selection

    -- GET THE ENVELOPE
    local retval, envelopeName = reaper.GetEnvelopeName(env, "envelopeName")
    local br_env = reaper.BR_EnvAlloc(env, false)

  -- TODO: this needs to be wrapped in a table, because this is super annoying
    ...
    if visible == true and armed == true then
        ...
        if env_points_count > 0 then
            for k = 0, env_points_count - 1 do
                retval, point_time, valueOut, shapeOutOptional, tensionOutOptional, selectedOutOptional =
                    reaper.GetEnvelopePoint(env, k)
      ...
      -- NOTE: why does he do this weth the micro ranges??
