...
---Get info about the whole chain - each subtable represents each FX slot.
...
fx_util.get_track_fx_info = function(tr, fx_idx)
  ...
  for i = 0, parm_cnt - 1 do
    ...
    local env = reaper.GetFXEnvelope(tr, fx_idx, i, false)
...
fx_util.fltr_all = function(opts) end

fx_util.fltr_fx_track_single = function(opts)
  opts = opts or {}
  ...
  -- loop effects
  local tr = opts.target_track

  if not tr then
    log.debug("[fx_util.fltr_fx_track_single]: No track `tr` (== nil)")
  end

  local t_track_fx_objs = {}
  ...
  for i = 0, tc - 1 do
    ...
    table.insert(t_track_fx_objs, fx_obj)
  end

  if filter then
    if type(filter) == "function" then
      log.user("[fx_util.fltr_fx_track_single]: filter == function")
      t_track_fx_objs = tbl.filter(t_track_fx_objs, filter)       -- pass filter func
    end
  end
  ...
  if opts.transform then
    for i, fxobj in ipairs(t_track_fx_objs) do
      local update = false
      if type(opts.transform) == "function" then
        opts.transform(fxobj)
        update = true
      end
      if update then
        -- i am not sure if this is useful to keep a counter
        fx_updated = fx_updated + 1
      end
    end
  end

  if fx_updated > 0 then
    -- TODO: here I could transform eg. the names of an FX?
  end

  return t_track_fx_objs
end
