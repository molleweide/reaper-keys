...
---@param data table
---@return table
lib_items.collect_single_item_data = function(item)
...
lib_items.all_project_items_filter_transform = function(opts)
  opts = opts or {}
  ...
  for _, _ in pairs(opts.filter) do
    filter_count = filter_count + 1
  end
  log.user("filter_count:", filter_count)

  local get_type = opts.get_type
  ...
  for i = 1, itemCount do
    ...
    -- Example of filtering in lib.midi
    -- t_notes = tbl.filter(t_notes, function(note)
    -- 	return subv[1] <= note[k] and note[k] <= subv[2]
    -- end)

    local filters_passed = 0
    if filter then
      -- if filter.range then
      --     if filter.range[1] <= pos and pos_end <= filter.range[2] then
      --         should_collect = true
      --     end
      -- end
      ...
      for k, v in pairs(opts.filter) do
        if k == "range" then
          if v[1] <= pos and pos_end <= v[2] then
            -- should_collect = true
            filters_passed = filters_passed + 1
          end
        elseif k == "of_tracks" then
          local item_tr = reaper.GetMediaItem_Track(item)
          local item_tr_guid = r.getGUIDByTrack(item_tr)
          for _, to in ipairs(v) do
            -- log.user(to.guid)
            if item_tr_guid == to.guid then
              filters_passed = filters_passed + 1
            end
          end
        end
    ...
    if filter_count == filters_passed then
      if get_type == "content" then
        local item_data = lib_items.collect_single_item_data(item)
        table.insert(t_res, item_data)
      end
    end
  end
...
function IsSelectionLinkEdit()                                     -- return bol
  link = toBits(reaper.SNM_GetIntConfigVar("midieditor", 5))
  [10]                                                             -- Is Selection is linked to editability On? 0 Yes 1 No.
  if link == 0 then
    link = true
  elseif link == 1 then
    link = false
...
--
-- Does this func return single or multiple items/takes in a reasonable manner??
lib_items.get_midi_item_takes_for_given_context = function(trobj, context, range_left, range_right)
