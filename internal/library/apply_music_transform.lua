...
---@return table
local function shift_and_loop_data_to_range(range_num, opts, t_midi_events, current_range)
  local res = {}

  local range_len = current_range.right - current_range.left
  ...
  for i = 0, num_loops - 1, 1 do
    for _, note in ipairs(t_midi_events) do
      ...
      new_note.time_pos_start = new_note.time_pos_start + current_range.left + loop_shift
      new_note.time_pos_end = new_note.time_pos_end + current_range.left + loop_shift
...
amt.apply_patterns_to_sel_tracks = function(opts)
  ...
  --
  -- If the opts table has a list of selected regions, and also we have a set
  -- of new regions opts.
  --
  local rd
  local target_ranges
  if custom_targets.regions and opts.add_new_region_opts then
    ...
    -- reverse loop insert new regions
    for i = #rd, 1, -1 do
      segments.inject_new_empty_region(rd[i])
    end

    opts.inject_new_regions = true
    target_ranges = rd
  else
    target_ranges = compute_target_timeline_ranges(custom_targets)
  end
  ...
  for range_num, t_target_range in ipairs(target_ranges) do
    -- note: Everything from here on, can depend on the cli options so I have
    -- to implement them one by one.

    if opts.inject_new_regions then
      local range_len = t_target_range.right - t_target_range.left
      t_target_range.left = t_target_range.left + (range_num - 1) * range_len
      t_target_range.right = t_target_range.right + (range_num - 1) * range_len
    end
    ...
    local music_data_shifted_to_position =
        shift_and_loop_data_to_range(range_num, opts, t_final_rendered_notes, t_target_range)
    ...
    for _, trnode in ipairs(target_tracks) do
      log.user("?")
      local target_item = check_if_item_exists_or_create(trnode.tr, t_target_range.left, t_target_range.right)
