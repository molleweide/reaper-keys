...
function segments.inject_space_at_range(pos_start, pos_end)
  ...
  r.set_time_sel(pos_start, pos_start + real_length)
  inject_space_at_time_sel()
...
function segments.duplicate_time_sel_data_at_shift_amount(tl_shift)
  ...
  segments.duplicate_items(data, tl_shift)   -- measure_shift+increment_measure)
end
...
function segments.duplicate_timeline_sel_data_at_cursor() end

---Get item data for selected media items.
---@param data table
---@return table
...
---Duplicate/copy items to a shifted position.
---@param data table as collected by `lib_items.all_project_items_filter_transform`
---@param measure_shift number
function segments.duplicate_items(data, measure_shift)
  for i = 1, #data do
