...
automation_actions.test = function()
  ...
  -- TODO: Get context and log existing/active curves.
  -- 1. get take at current position.
  -- 2. list active env curves
  -- 3. list active cc curves.
  -- --
  -- -- list track env info
  -- --
  -- local count_envs = reaper.CountTrackEnvelopes( track )
  -- -- loop
  -- local track_env = reaper.GetTrackEnvelope( track, envidx )
  -- -- get all these
  -- reaper.GetEnvelopeInfo_Value( env, parmname )
  -- --
  -- LOOP AND COLLECT CC EVENTS.
  -- retval, selected, muted, ppqpos, chanmsg, chan, msg2, msg3 = reaper.MIDI_GetCC( take, ccidx )
  -- This is a thread that shows how one can do this:
  --     https://forum.cockos.com/showthread.php?t=272051&highlight=get+all+midi+cc+events+for+lane
  --     >> i need to handle pitch and some specific CC mesagges differently.
  --
