          ...
          for i, pt in ipairs(sel.def) do
            ...
            if i == 1 then
              ...
              table.insert(t_pts_to_insert,
                { name = "start_post_sig", position = range_left + 1 * env_step_delta, param_val = pt.val })
            elseif i == #t_pts_to_insert then
              log.user("endpoint")
              table.insert(t_pts_to_insert,
                { name = "end_pre_sig", position = range_right - 2 * env_step_delta, param_val = pt.val })
              ...
              log.user(string.format([[%s / %s / %s]], range_length, #sel.def, range_length / (#sel.def - 1)))
              ...
              table.insert(t_pts_to_insert, { name = "mid", position = normalized_curve_position, param_val = pt.val })
            end
          end
