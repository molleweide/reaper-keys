...
---Iterate the MIDI messages inside the string out of  reaper.MIDI_GetAllEvts.
--Returns offset, flags, msg, offset_count, stringPos. offset is offset from
--last midi event. flags is midi message reaper option like muted, selected....
--msg is the midi message. offset_count is the offset in ppq from the start of
--the iteration (the start of the MIDI item or at start.stringPos(start.ppq and
...
---Use to delete elements that existed previusly, without needing to change any
--offset. Can also use SetMIDIUnsorted with a '' (empty string) at the midi_msg
--value. Can insert it a table as last position or change some element to
--placeholder(that will get deleted) in the list. If going to change put the
--pos of the element and dont put offset and offset_count
...
--- Return a table with selected notes info. Per Item pitch = selected pitches.
--.interval = interval between notes. .vel = velocities. .len = length of the
--notes. .rhythm = ppq difference between notes. .groove = tables for each
--measured with notes copied, ppq diference from measure start and each note.
---@param take any
...
---Create a table contaning all selected MIDI note events; This Function don't
--work well with looped MIDI items, because idk how would be a good design for
--that situation. Should it get the notes in the first loop info? or at the
--longer loop? or at the last loop? The only difference is time, if I decided
--add the offset start to the start position of each note and multiply the
--notes.
...
--- Simillar function as CopyMIDIParameters but is to be used with
--EventTogether, simillar function return the list of  pitch, interval, rhythm,
--measure_pos , velocity, len, pos used. Pos, rhythm, measure_pos, len are also
--available in QN, project QN based, instead of calculating per item ppq, good
--for cases where one item is with different playbackrate. Usefull for mixing
--takes. -@param take_table table table with all events tables with all midi
--notes info to get the parameters from. this take table can actually be a
--mixture made in EventTogether. Created in CreateNotesTable. structure of each
--note table is : offset_count = note start in ppq, endppqpos = note end in
--ppq, chan = chan, pitch = pitch, vel = vel, start_time = proj start_time in
--sec -@param mark_muted_pitch boolean mark the parameters that comes from
--muted notes for pitch/intervals. for intervals it will mark if the second
--note is muted. -@param mark_muted_rhythms boolean mark the parameters that
--comes from muted notes. for rhythms it will mark if the second note is muted.
---@param mark_muted_len boolean mark the parameters that comes from muted
--notes. -@param mark_muted_vel boolean mark the parameters that comes from
--muted notes. -@param mark_muted_pos boolean mark the parameters that comes
--from muted notes. -@param mute_mark string identifier to be added to muted
--parameters -@return table copy_list  copy_list.pitch = {{60,64,67},{60},{59}}
--C major followed by C B. copy_list.rhythm =
--{{0,25,60},{960,50,12},{1080},{1080}}. If rhythm or interval start with a
--event with more than one notes then the values between the notes of the event
--will be insert at [0] , copy_list.interval[0] = {{3,4}},
--copy_list.interval[1] = {{2,3,4}}, copy_list.interval[2] = {{5}}, in this
--case it starts with a minor chord goes 2 semitones up do anothe minor chord
--and makes a note 5 semitones up. Two chords at the first two events distance
--between events starts is the first value in a event table, the other values
--are for notes inside one element. The is also interval, groove, velocity,
--len.
function CopyMIDIParametersFromEventList(take_table,
  mark_muted_pitch,mark_muted_rhythms,mark_muted_len,mark_muted_vel,
  mark_muted_pos, mute_mark)
