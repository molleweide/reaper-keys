...
function string_funcs.jStringExplodeOld(s, sep)
...
function string_funcs.jStringExplode(s, sep, bIgnoreCase)
	-- Explode a string by seperator
	-- Returns #1: Table with parts
	-- Returns #2: Amount of times the seperator can be found
	-- If the seperator is NOT found it will return a table with 1 element and 0.
...
function string_funcs.jStringTrim(s)
	return s:match("^%s*(.-)%s*$")
end

function string_funcs.jStringIsInt(s)
	local r = s:match("^(%d+)$")
	return r ~= nil
end
