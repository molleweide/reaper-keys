...
function VF_GetFormattedGrid(grid_div)
	...
	if denom >= 2 then
		...
		if is_triplet then
			grid_str = "1/" .. math.floor(denom * 2 / 3)
		end
	else
		grid_str = 1
...
function getKeysSortedByValue(tbl, sortFunction, param) -- https://stackoverflow.com/questions/2038418/associatively-sorting-a-table-by-value-in-lua
	...
	table.sort(keys, function(a, b)
		return sortFunction(tbl[a][param], tbl[b][param])
	end)
	return keys
end
...
function eugen27771_GetObjStateChunk(obj)
	...
	if vrs_num >= 5.93 then
		...
		if SNM_GetSetObjectState(obj, fast_str, false, false) then
			chunk = SNM_GetFastString(fast_str)
		end
...
end --https://github.com/majek/wdl/blob/master/WDL/db2val.h
...
function GetSpectralData(item)
	...
	for line in chunk:gmatch("[^\r\n]+") do
		...
		if line:match("SPECTRAL_EDIT%s") then
			if not SE[tk_cnt].edits then
				SE[tk_cnt].edits = {}
			end
			local tnum = {}
			for num in line:gmatch("[^%s]+") do
				if tonumber(num) then
					tnum[#tnum + 1] = tonumber(num)
				end
			end
			...
			SE[tk_cnt].edits[#SE[tk_cnt].edits + 1] = {
		...
		if line:match("SPECTRAL_EDIT_B") then
			if not SE[tk_cnt].edits[#SE[tk_cnt].edits].points_bot then
				SE[tk_cnt].edits[#SE[tk_cnt].edits].points_bot = {}
			end
			for pair in line:gmatch("[^%+]+") do
				SE[tk_cnt].edits[#SE[tk_cnt].edits].points_bot[#SE[tk_cnt].edits[#SE[tk_cnt].edits].points_bot + 1] =
					pair:match(pat)
			end
		end

		if line:match("SPECTRAL_EDIT_T") then
			if not SE[tk_cnt].edits[#SE[tk_cnt].edits].points_top then
				SE[tk_cnt].edits[#SE[tk_cnt].edits].points_top = {}
			end
			for pair in line:gmatch("[^%+]+") do
				SE[tk_cnt].edits[#SE[tk_cnt].edits].points_top[#SE[tk_cnt].edits[#SE[tk_cnt].edits].points_top + 1] =
					pair:match(pat)
			end
		end
	end
...
function SetSpectralData(item, data, apply_chunk)
	...
	for i = 1, #t do
		...
		if open and t[i]:match(">") then
			...
			if data[tk_cnt] and data[tk_cnt].edits and take and not TakeIsMIDI(take) then
				for edit_id in pairs(data[tk_cnt].edits) do
					if not data[tk_cnt].FFT_sz then
						data[tk_cnt].FFT_sz = 1024
					end
					local s_offs = GetMediaItemTakeInfo_Value(take, "D_STARTOFFS")
					local rate = GetMediaItemTakeInfo_Value(take, "D_PLAYRATE")
						...
						if data[tk_cnt].edits[edit_id].points_bot then
							for pt = 1, #data[tk_cnt].edits[edit_id].points_bot, 10 do
								add_str = add_str
									.. "SPECTRAL_EDIT_B "
									.. table.concat(
										data[tk_cnt].edits[edit_id].points_bot,
										" + ",
										pt,
										math.min(#data[tk_cnt].edits[edit_id].points_bot, pt + 10)
									)
							end
							add_str = add_str .. "\n"
						end
						...
						if data[tk_cnt].edits[edit_id].points_top then
							for pt = 1, #data[tk_cnt].edits[edit_id].points_top, 10 do
								add_str = add_str
									.. "SPECTRAL_EDIT_T "
									.. table.concat(
										data[tk_cnt].edits[edit_id].points_top,
										" + ",
										pt,
										math.min(#data[tk_cnt].edits[edit_id].points_top, pt + 10)
									)
							end
							add_str = add_str .. "\n"
						end
...
function ExtState_Save(conf)
	...
	for key in pairs(conf) do
		SetExtState(conf.ES_key, key, conf[key], true)
	end
...
---------------------------------------------------------------------------------------------------------------------
function math_q(num)
	if math.abs(num - math.floor(num)) < math.abs(num - math.ceil(num)) then
...
function GetInput(conf, captions_csv, retvals_csv, floor, linew, is_string)
	if linew and tonumber(linew) then
		captions_csv = captions_csv .. ",extrawidth=" .. math.floor(linew)
	end
...
function BinaryCheck(value, byte_id, bool_int)
	...
	if value & byte_num == byte_num then
		if not bool_int or (bool_int and bool_int == 1) then
			value = value - byte_num
		end
	else
		if not bool_int or (bool_int and bool_int == 0) then
			value = value + byte_num
		end
	end
	return value
...
function VF_SetFXName(track, fx, new_name)
	...
	if VF_CheckReaperVrs(6.78) then
		TrackFX_SetNamedConfigParm(track, fx, "renamed_name", new_name)
		return
	end

	local edited_line, edited_line_id, segm
	-- get ref guid
	if not track or not tonumber(fx) then
		return
	end
...
function HasWindXYWHChanged(obj)
	...
	local retval = 0
	if not obj.last_gfxx or not obj.last_gfxy or not obj.last_gfxw or not obj.last_gfxh or not obj.last_dock then
		obj.last_gfxx, obj.last_gfxy, obj.last_gfxw, obj.last_gfxh, obj.last_dock = wx, wy, ww, wh, dock
		return -1
	end
	if wx ~= obj.last_gfxx or wy ~= obj.last_gfxy then
		retval = 2
	end --- minor
	if ww ~= obj.last_gfxw or wh ~= obj.last_gfxh or dock ~= obj.last_dock then
		retval = 1
	end --- major
	obj.last_gfxx, obj.last_gfxy, obj.last_gfxw, obj.last_gfxh, obj.last_dock = wx, wy, ww, wh, dock
	return retval
...
function Menu(mouse, t)
	...
	if ret > 0 then
		for i = 1, ret do
			if t[i + incr].menu_decr == true then
				incr = incr - 1
			end
			if t[i + incr].str:match(">") then
				incr = incr + 1
			end
			if t[i + incr].menu_inc then
				incr = incr + 1
			end
		end
...
function VF_ReduceFXname(s)
	...
	local pat_js = ".*[%/](.*)"
	if s_out:match(pat_js) then
		s_out = s_out:match(pat_js)
	end
	if not s_out then
		return s
	else
...
function VF_SetTimeShiftPitchChange(item, get_only, pshift_mode0, timestr_mode0, stretchfadesz)
	...
	for playratechunk in str:gmatch("(PLAYRATE .-\n)") do
		...
		if pshift_mode0 then
			t[4] = pshift_mode0
		end
		if timestr_mode0 then
			t[5] = timestr_mode0
		end
		if stretchfadesz then
			t[6] = stretchfadesz
		end
		local playratechunk_out = "PLAYRATE " .. table.concat(t, " ") .. "\n"
		str_mod = str_mod:gsub(
			playratechunk:gsub("[%.%+%-]", function(c)
				return "%" .. c
			end),
			playratechunk_out
		)
...
function VF_GetActionCommandIDByFilename(searchfilename) -- https://forum.cockos.com/showpost.php?p=2383082&postcount=6
	...
	for i = 1, #t do
		if t[i].filename:match(searchfilename) then
			return t[i].aid
		end
	end
end
...
function VF_MenuReturnAction(MOUSE, OBJ, DATA, str_name, func0)
	return { str = str_name, func = func0 }
end
...
function VF_ExtState_Load(conf, preset)
	...
	if not PRESET_str64 or PRESET_str64 == "" then
		VF_ExtState_Save(conf, 0)
	end
	...
	if PRESET_str64 and PRESET_str64 ~= "" then
		...
		for line in PRESET_str:gmatch("[^\r\n]+") do
			...
			if key and value and key ~= "" and value ~= "" then
				preset_t[key] = tonumber(value) or value
			end
		end
	end
...
function VF_ExtState_SaveProj(conf, extname)
	for key in spairs(conf) do
		SetProjExtState(0, extname, key, conf[key])
	end
end

------------------------------------------------------------------------------------------------------
function VF_Open_URL(url)
...
function VF_GetMediaTrackByGUID(optional_proj, GUID)
	...
	local mast = reaper.GetMasterTrack(optional_proj0)
	if reaper.GetTrackGUID(mast) == GUID then
		return mast
	end
end
...
function VF_ConvertNoteOnVel0toNoteOff(take)
	...
	while stringPos < MIDIlen - 12 do
		...
		if msg1:len() == 3 then
			...
			if msgb1 & 0xF0 == 0x90 and msg1:byte(3) == 0 then
				msgb1 = 0x80 | (msg1:byte(1) & 0xF)
			end
...
function VF_AnalyzeItemLoudness(item) -- https://forum.cockos.com/showpost.php?p=2050961&postcount=6
	...
	-- analyze
	local success = reaper.NF_AnalyzeMediaItemPeakAndRMS(
		item,
		windowSize,
		reaperarray_peaks,
		reaperarray_peakpositions,
		reaperarray_RMSs,
		reaperarray_RMSpositions
	)
...
function VF2_GetSetFXChunk(track, fx, strreplace) -- from BYPASS to WAK -- 15.05.2021
	...
	for fxchunk in trchunk:gmatch("(BYPASS.-WAK %d)") do
		...
		if not fxGUID then
			fxGUID = fxchunk:match("FXID_NEXT (.-)\n")
		end
	...
	if not FXchunk_find then
		return
	end
	if strreplace then
		trchunk = trchunk:gsub(FXchunk_find, strreplace)
...
function VF2_GetMEZoom(take)
	...
	for line in chunk:gmatch("[^\r\n]+") do
		if line:match("GUID (.*)") then
			...
			if testtake and testtake == take then
				active_take = true
			end
		end
...
function VF2_ShiftRegions(offset)
	...
	for i = 1, #regions do
		AddProjectMarker2(
			0,
			regions[i].isrgn,
			regions[i].pos + offset,
			regions[i].rgnend + offset,
			regions[i].name,
			regions[i].markrgnindexnumber,
			regions[i].color
		)
	end
	return true
...
function VF2_EnumeratePlugins_Sub(plugs_t, fp, pat, plugtype)
	...
	for line in content:gmatch("[^\r\n]+") do
		...
		if str then
			...
			if plugtype == 3 then
				if reduced_name:match("%:.*") then
					reduced_name = reduced_name:match("%:(.*)")
				end
			end
			if plugtype == 4 then
				--reduced_name = reduced_name:sub(5)
				local pat_js = ".*[%/](.*)"
				if reduced_name:match(pat_js) then
					reduced_name = reduced_name:match(pat_js)
				end
			end
			plugs_t[#plugs_t + 1] = { name = str, reduced_name = reduced_name, plugtype = plugtype }
		end
	end
end
...
function VF2_CollectFXData()
	...
	for i = 1, CountTracks(0) do
		...
		for fx = 1, fxcnt do
			...
			if not pluginsdata[buf] then
				pluginsdata[buf] = {}
			end
			local GUID = TrackFX_GetFXGUID(tr, fx - 1)
			pluginsdata[buf][#pluginsdata[buf] + 1] = { tr_ptr = tr, name = buf, pos = fx - 1, GUID = GUID }
		end
	end
	return pluginsdata
end
...
function VF2_IsTrackSendExists(src_tr, dest_tr0)
	for sid = 1, GetTrackNumSends(src_tr, 0) do
		local dest_tr = GetTrackSendInfo_Value(src_tr, 0, sid - 1, "P_DESTTRACK")
		if GetTrackGUID(dest_tr) == GetTrackGUID(dest_tr0) then
			return true
		end
	end
end
...
function VF_spk77_getinivalue(ini_file_name, section, key) -- https://forum.cockos.com/showpost.php?p=1535873&postcount=8
	...
	local function VF_spk77_get_ini_value_startswith(text, prefix)
		return string.sub(text, 1, string.len(prefix)) == prefix
	end

	local function VF_spk77_get_ini_value_split(s, sep)
		return s:match("([^" .. sep .. "]+)[" .. sep .. "]+(.+)")
	end

	local function VF_spk77_get_ini_value_trim(s)
		return s:match("^%s*(.-)%s*$")
	end
	...
	for line in content:gmatch("[^\r\n]+") do
		...
		if section_found and line == "%[.*%]" then
			break
		end -- break at next section
	...
	if not key_found then
		if section_found and not key_found then
			reaper.ShowConsoleMsg("Couldn't find key: " .. key .. "\n")
		end
		return false
	end
end
	...
	for fx = 0, TrackFX_GetCount(track) - 1 do
		if fx0 == -1 or (fx0 ~= -1 and fx0 == fx) then
			...
			if wakID and t[wakID] then
				...
				if not src_line:match("WAK [%d%-]+ [%d%-]+") then
					src_line = src_line .. " 0"
				end -- backward comatibility with old reaper versions
...
function VF_BFpluginparam(find_Str, tr, fx, param)
	...
	if not (find_Str_val and tonumber(find_Str_val)) then
		return
	end
...
function VF_BFpluginparam_GetFormattedParamInternal(tr, fx, param, val)
	...
	if param_str then
		param_n = tonumber(param_str)
	end
	if not param_n and param_str:lower():match("%-inf") then
		param_n = -math.huge
	elseif not param_n and param_str:lower():match("inf") then
		param_n = math.huge
	end
	return param_n
end
...
function GetNoteStr(conf, val, mode) -- conf.key_names
	local oct_shift = -1
	if conf.oct_shift then
		oct_shift = conf.oct_shift
	end
	local int_mode
	if mode then
		int_mode = mode
	else
		int_mode = conf.key_names
	end
	if int_mode == 0 then
		if not val then
			return
	...
	elseif
		int_mode == 4 -- midi pitch
	then
		return val, "MIDI pitch"
	elseif
		int_mode == 5 -- freq
	then
		return math.floor(440 * 2 ^ ((val - 69) / 12)) .. "Hz"
	elseif
		int_mode == 6 -- empty
	then
		return "", "Nothing"
	elseif int_mode == 7 then -- ru
		...
		if note and oct and key_names[note + 1] then
			return key_names[note + 1] .. oct + oct_shift, "keys (RU) + octave"
		end
...
function VF_GetNoteStr(val, int_mode) -- conf.key_names
	...
	elseif int_mode == 7 then -- ru
		...
		if note and oct and key_names[note + 1] then
			return key_names[note + 1] .. oct, "keys (RU) + octave"
		end
	elseif int_mode == 8 then
		if not val then
			return
...
function VF_GetClosestPrevNextGridDivision(pos)
	...
	local retval1, timepos, measurepos, beatpos, bpm, timesig_num, timesig_denom, lineartempo =
		GetTempoTimeSigMarker(0, tsmarker)
	...
	if is_next_closest == true then
		closest_div_pos = next_div_pos
	end
