		...
		m = {
			...
			[2] = {
				...
				named_config_params = function(opts_g, t_rsfx)
					...
					--
					-- check if sampler already has loaded sample
					--

					--
					-- MV TO UTILS PATTERNS
					--

					local wav_ext_pattern = "%.[wW][aA][vV]$"

					--
					-- CHECK IF SAMPLERS ALREADY ARE LOADED WITH SAMPLES
					--
					...
					if buf:find(wav_ext_pattern) ~= nil then
						...
						--
						-- mv this to utils > strings ?
						--
						-- note: this should return a table and then we can use each indice
