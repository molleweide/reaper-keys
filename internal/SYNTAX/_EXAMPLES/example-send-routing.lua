...
function new_midi_send(src, dest) ---------------------------------------------------------------------------------------
  local dest_recv_count = reaper.GetTrackNumSends(dest, TRACK_INFO_RECV_CATEGORY) -- target receive count; how many sends go b target
  local dest_highest_midi_recv = 0 -- this is needed bc receives can be audio you know...

  for i = 0, (dest_recv_count - 1) do --:::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    local midi_flags = reaper.GetTrackSendInfo_Value(dest, TRACK_INFO_RECV_CATEGORY, i, "I_MIDIFLAGS") -- get current midi flags for destination track

    -- if receive is not other than midi ... right?!
    if midi_flags == TRACK_INFO_MIDIFLAGS_DISABLED then
      goto continue
    end

    -- keep count of num midi receives. this is why you cannot use original receive count
    -- i don't understand how this is done.
    --  i think this gives the highest already being used channel
    --      so that this can be used for creating the new incremented midi channel data
  ...
  local midi_send = reaper.CreateTrackSend(src, dest) -- create send; return sendidx for reference
  local new_midi_flags = create_send_flags(0, dest_highest_midi_recv + 1) -- create new incremented midi channel spec
  reaper.SetTrackSendInfo_Value(src, TRACK_INFO_SEND_CATEGORY, midi_send, "I_MIDIFLAGS", new_midi_flags) -- set midi_flags on reference
  reaper.SetTrackSendInfo_Value(src, TRACK_INFO_SEND_CATEGORY, midi_send, "I_SRCCHAN", TRACK_INFO_AUDIO_SRC_DISABLED) -- turn or of audio
  return midi_send
end ---------------------------------------------------------------------------------------------------------------------
