...
function sends.createSend(src_obj,dest_obj,dest_chan)
  ...
  -- log.user(dest_obj)
  local src_obj_trk, src_idx  =  util.VF_GetTrackByGUID(src_obj.guid)-- reaper.GetTrack(0,src_obj.trackIndex)
  local dest_obj_trk, dst_idx = util.VF_GetTrackByGUID(dest_obj.guid)--reaper.GetTrack(0,dest_obj.trackIndex)
...
function sends.removeAllSends(trk_obj)
  local tr, tr_idx = util.VF_GetTrackByGUID(trk_obj.guid)--reaper.GetTrack(0,trk_obj.trackIndex)
